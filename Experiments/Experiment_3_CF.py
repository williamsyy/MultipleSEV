...
for iter in range(args.iterations):
    ...
    for i in tqdm(range(factual.shape[0])):
        ...
        L_0_gs = np.sum(sev.data_map.dot(abs((gs_facts.values-factual.iloc[[i]].values).T!=0))>0)
        # L_0_cchvae = np.sum(sev.data_map.dot(abs((cchvae_facts.values-factual.iloc[[i]].values).T>1e-2))>0)
        L_0_dice = np.sum(sev.data_map.dot(abs((dice_facts.values-factual.iloc[[i]].values).T!=0))>0)
        L_0_revise = np.sum(sev.data_map.dot(abs((revise_facts.values-factual.iloc[[i]].values).T!=0))>0)
        L_0_wachter = np.sum(sev.data_map.dot(abs((wachter_facts.values-factual.iloc[[i]].values).T!=0))>0)
        # labels
        GS_lst.append(sev.data_map.dot(abs((gs_facts.values-factual.iloc[[i]].values).T!=0)).reshape(-1))
        DiCE_lst.append(sev.data_map.dot(abs((dice_facts.values-factual.iloc[[i]].values).T!=0)).reshape(-1))
        Revise_lst.append(sev.data_map.dot(abs((revise_facts.values-factual.iloc[[i]].values).T!=0)).reshape(-1))
        Wachter_lst.append(sev.data_map.dot(abs((wachter_facts.values-factual.iloc[[i]].values).T!=0)).reshape(-1))
    ...
    embedding = LocalMAP()
    GS_embed = embedding.fit_transform(np.array(GS_lst)[i for i in GS_lst if np.isnan(i).any()])
    embedding = LocalMAP()
    DiCE_embed = embedding.fit_transform(np.array(DiCE_lst)[i for i in DiCE_lst if np.isnan(i).any()])
    embedding = LocalMAP()
    Revise_embed = embedding.fit_transform(np.array(Revise_lst)[i for i in Revise_lst if np.isnan(i).any()])
    embedding = LocalMAP()
    Wachter_embed = embedding.fit_transform(np.array(Wachter_lst)[i for i in Wachter_lst if np.isnan(i).any()])
    ...
    plt.savefig("SEV_{}.png".format(args.dataset))
    plt.close()
