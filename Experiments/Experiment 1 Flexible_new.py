...
specific_sev = "../Results/csv/Exp1_flexible/Exp1_{}_{}_{}_flexible_new.csv".format(args.dataset,args.model,args.tolerance)
overall_results = "../Results/csv/Exp1_flexible_new_summary.csv"
...
for iter in range(args.iterations):
    ...
    if os.path.isfile(overall_results):
        ...
        overall_results_df = pd.DataFrame(columns=["Dataset","Model","Tolerance","Iteration","Train AUC","Test AUC","Train Accuracy","Test Accuracy","Average SEV","Average L_inf","Average Time","Proportion of using flexible SEV"])
        overall_results_df.to_csv(overall_results,index=False)
    overall_results_df.loc[len(overall_results_df)] = [args.dataset,args.model,args.tolerance,iter,train_auc,test_auc,train_acc,test_acc,flexible_sev[flexible_sev!=0].mean(),np.mean(L_inf),np.mean(time_lst)* 100,np.mean(used_lst)]
    overall_results_df.to_csv(overall_results,index=False)
