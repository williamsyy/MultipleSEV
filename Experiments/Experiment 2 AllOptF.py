...
parser.add_argument('--warm_up', type=float, default=0.7, help='warm up ratio')
...
specific_sev = "../Results/csv/Exp2_flexible_opt/Exp2_{}_{}_{}_flexopt_new.csv".format(args.dataset,args.model,args.tolerance)
overall_results = "../Results/csv/Exp2_flexopt_summary_new.csv"
        ...
        for positive_penalty in positive_penalty_lst:
            ...
            for i in tqdm(range(X_test.shape[0])):
                ...
                if base_model.predict(Xi) == 1:
                    ...
                    explanations = sev.sev_explain(Xi[0],sev_value,mode="minus")
                    sev_lst.append(sev_value)
                    time_lst.append(time.time()-start_time)
                    # L_inf_lst.append(np.min([np.max(np.abs(explain)) for explain in explanations]))
