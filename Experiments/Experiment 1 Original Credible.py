...
import numpy as np
import pandas as pd
from FlexibleSEV_credit import FlexibleSEV as FlexibleSEVCredit
...
for iter in range(args.iterations):
    ...
    print("The average GMM score (previous) is {}".format(np.mean(gmm_lst)))

    # build the original SEV method
    originalSEV = FlexibleSEVCredit(model,encoder,encoded_X.columns,encoded_X_neg,tol=0,k=1,n_components=cluster_dictionary[args.dataset],threshold=np.quantile(gmm_lst,0.75))
    ...
    for ind,xi in enumerate(tqdm(np.array(X_test))):
        ...
        #     print(explanation)
        # L_inf.append(np.abs(original_diff[(original_diff!=0)&(original_diff!=1)&(original_diff!=-1)]).max())
    ...
    print(pd.Series(original_sev).value_counts().sort_index())
    # print("The average time for the original SEV method is {}".format(np.mean(time_lst)* 100))
    print("The average GMM score (new) is {}".format(np.mean(gmm_lst)))
