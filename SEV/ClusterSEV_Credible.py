...
class ClusterSEV:
    def __init__(self, model, data_encoder, X_col, X_neg, n_clusters = 3, m=2, strict=None,threshold=0):
        self.model = model
        ...
        self.gmm = GaussianMixture(n_components=n_clusters, random_state=42).fit(X_neg)
    ...
    def sev_cal(self,Xi, X_emb, mode = "plus",max_depth=None):
        ...
        for choice in  range(1,len(self.data_encoder.original_columns)+1):
            ...
            for comb in combs:
                ...
                if mode == "minus":
                    if score < 0.5:
                        if self.gmm.score_samples(X_trans) > self.threshold:
                            return len(comb),X_trans-Xi
                        if 'temp_result' not in locals():
                            temp_result = X_trans
                            temp_sev = len(comb)
                            final_sev = len(comb)
                            temp_score = score
                        elif np.abs(len(comb) - final_sev) <= 1 and score > temp_score:
                            temp_result = X_trans
                            temp_score = score
                            temps_sev = len(comb)
                else:
                    if score >= 0.5:
                        ...
                        if 'temp_result' not in locals():
                            ...
                            temp_score = score
                            temp_sev = len(comb)
                        elif np.abs(len(comb) - final_sev) > 1:
                            break
                        
        return temp_sev, temp_result - Xi
